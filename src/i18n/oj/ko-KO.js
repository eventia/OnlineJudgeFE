export const m = {
  // Problem.vue
  Description: '설명',
  Input: '입력',
  Output: '출력',
  Sample_Input: '입력 예',
  Sample_Output: '출력 예',
  Hint: '힌트',
  Source: '소스코드',
  Status: '상태',
  Information: '정보',
  Time_Limit: '시간 제한',
  Memory_Limit: '메모리 제한',
  Created: '생성됨',
  Level: '레벨',
  Score: '점수',
  Tags: '태그',
  Show: '보기',
  // About.vue
  Result_Explanation: '결과 설명',
  Pending_Juding: '준비중...',
  Compile_Error: '컴파일에러, 오류 내용을 확인하세요.',
  Accepted: '정답입니다.',
  Wrong_Answer: '오답입니다.',
  Runtime_Error: '런타임오류로 프로그램이 비정상적으로 종료됨. 0으로 나누기 오류, 권한 이상의 하드웨어 접근 등 치명적인 논리 오류가 있습니다.',
  Time_Limit_Exceeded: '시간 초과입니다(Java와 Python 은 시간 관리를 잘 하셔야 합니다).',
  Memory_Limit_Exceeded: '메모리 초과입니다.',
  System_Error: '채점 시스템에 오류가 발생했습니다. 관리자에게 문의하세요.',
  // FAQ.vue
  Frequently_Asked_Questions: '자주 물어보는 질문들',
  // ContestDetail.vue
  Problems: '문제',
  Announcements: '공지사항',
  Submissions: '기존에 풀어둔 결과',
  Rankings: '순위',
  Overview: '개요',
  Admin_Helper: '관리자 도움',
  // ContestProblemList
  Problems_List: '문제 목록',

  // NavBar.vue
  Home: '홈',
  NavProblems: '문제',
  Contests: '대회',
  NavStatus: '상태',
  Rank: '순위',
  ACM_Rank: 'ACM 순위',
  OI_Rank: 'OI 순위',
  About: '개요',
  Judger: '채점 시스템',
  FAQ: '자주하는질문',
  Login: '로그인',
  Register: '회원가입',
  MyHome: '홈',
  MySubmissions: '제출',
  Settings: '설정',
  Management: '관리',
  Logout: '로그아웃',
  // announcements.vue
  Refresh: '새로고침',
  Back: '뒤로가기',
  No_Announcements: '공지사항 없음',
  // Setting.vue
  Profile: '프로필',
  Account: '계정',
  Security: '보안',
  // AccoutSetting.vue
  ChangePassword: '비밀번호 변경',
  ChangeEmail: '이메일 변경',
  Update_Password: '비밀번호 업데이트',
  // ProfileSetting.vue
  Avatar_Setting: '아바타 설정',
  Profile_Setting: '프로필 설정',
  // SecuritySettig
  Sessions: '세션',
  Two_Factor_Authentication: '이중 인증',
  // Login.vue
  LoginUsername: '아이디',
  LoginPassword: '비밀번호',
  TFA_Code: 'TFA 코드를 입력하세요.',
  No_Account: '계정이 없으신가요? 회원가입하세요!',
  Forget_Password: '비밀번호를 잊으셨나요?',
  UserLogin: '로그인',
  // Register.vue
  RegisterUsername: '아이디',
  Email_Address: '이메일 주소',
  RegisterPassword: '비밀번호 입력',
  Password_Again: '비밀번호 재입력',
  Captcha: '캡차',
  UserRegister: '회원가입',
  Already_Registed: '등록되어있습니다. 로그인하세요.',
  // ResetPassword.vue and ApplyResetPassword.vue
  Reset_Password: '비밀번호 분실',
  RPassword: '비밀번호',
  RPassword_Again: '비밀번호 확인',
  RCaptcha: '캡차',
  ApplyEmail: '이메일 주소',
  Send_Password_Reset_Email: '비밀번호 재전송을 위한 이메일 주소',
  // UserHome.vue
  UserHomeSolved: '해결',
  UserHomeserSubmissions: '제출',
  UserHomeScore: '점수',
  List_Solved_Problems: '푼 문제들',
  UserHomeIntro: '아직 어떠한 문제도 풀지 않았습니다.'
}
